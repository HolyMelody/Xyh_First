...
class Topo(object):
    ...
    def Draw_path(self,list1,list2):
        if list1!=[]:
            G = nx.Graph()
    ...
    def shortest_path(self, src_sw, dst_sw, first_port, last_port):
        ...
        return record, short ,long,list1,list2
...
class DFSController(app_manager.RyuApp):
    ...
    def packet_in_handler(self, event):
        ...
        if dst_mac in self.mac_to_port.keys():
            ...
            result,a,b,c,d= self.topo.shortest_path(src_switch,dst_switch,in_port,final_port)
            self.falg = True
            ...
            #if self.n==500:##pingall截断测试
                    #self.topo.Draw_path(c,[])
                    #plt.show()
            # current_switch=None
            #print("图片显示3秒")
            # self.topo.Draw_path([],d)
