...
class bucket(object):
    ...
    def __init__(self,ration):
        self.ration=ration#桶的容量
        # print(self.ration)
        self.data=[[] for i in range(0,ration)]
        self.front=0#桶前指针
 
    def insert(self,newdata):
        #i=int((newdata[0])%self.ration)
        i=self.index(newdata[0])
        self.data[i].append(newdata)
    def index(self,data):
        return int(data%(self.ration))#data%(self.ration)
    def Exact_min(self): #取出离桶前指针最近的桶中第一个元素
       while not self.data[self.index(self.front)]:
                  self.front=self.front+1
       key=self.data[self.index(self.front)].pop(0)
       #[[(6, 0, 2)], [(7, 3, 2), (7, 3, 4)], [(8, 3, 5)], [], [(4, 1, 2)], []]
       #self.data[self.front % self.ration].pop(0)
       if(len(self.data[self.index(self.front)]))==0:
            self.front=self.index(self.front)
       return key
class Topo(object):
    ...
    def get_graph(self):
        I=9
        Matrix = np.array(np.ones((20,20))*I)
        ...
        return edges
    def draw_graph(self,graph,path,weight):
        ...
        plt.savefig("p_3.png")
        #plt.show()
    def dijkstra(self,src_sw,dst_sw,first_port,last_port):
        ...
        #开始
        flag=1
        while len(visited)!=20 and flag==1:   #len(nodes):遍历所有节点
        ...
        for a in self.switches:
            for b in self.switches:
                if self.get_adjacent(a,b) is not None:
                    graph.append((a,b))
                    weight[(a,b)] = self.get_adjacent(a,b)[1]
        graph_path=[]
        for i in range(len(path_list[dst_sw])-1):
            graph_path.append((path_list[dst_sw][i],path_list[dst_sw][i+1]))
            graph_path.append((path_list[dst_sw][i+1],path_list[dst_sw][i]))
        self.draw_graph(graph,graph_path,weight)
        print("权重和：")
        print(path[dst_sw])
        print("路径：")
        print(path_list[dst_sw])
        if src_sw==dst_sw:
                path=[src_sw]
        else:
                path=path_list[dst_sw] 
        record=[]
        inport=first_port
            # s1 s2; s2:s3, sn-1  sn
        for s1,s2 in zip(path[:-1],path[1:]):
                # s1--outport-->s2
            if (self.get_adjacent(s1,s2) is not None):
