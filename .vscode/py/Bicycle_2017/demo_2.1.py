...
# 预处理数据
df['ds'] = pd.to_datetime(df.iloc[:, 0], unit='s')
...
# 将结果四舍五入为整数
result_df.iloc[:, 1:] = result_df.iloc[:, 1:].round().apply(pd.to_numeric, errors='ignore')

# 确保结果大于0
result_df.iloc[:, 1:] = result_df.iloc[:, 1:].clip(lower=1)

result_df.to_csv(os.path.join(output_dir, "Forecast.csv"), index=False, encoding='utf-8-sig')
...
# 将结果四舍五入为整数
total_rentals = [(x) for x in total_rentals]
total_rentals = list(map(int, total_rentals))

# 确保结果大于0
total_rentals = [max(x, 1) for x in total_rentals]
